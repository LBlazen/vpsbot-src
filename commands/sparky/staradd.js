...
run: async (client, message, args) => {
    ...
    // add coins
    mongoCurrency.giveCoins(user.id, message.guild.id, amount);
    // send message
    message.channel.send(`${user.tag} now has ${amount} coins.`);

    // log
    console.log(`${message.author.tag} added ${amount} coins to ${user.tag}`);
    // log any errors
}
}